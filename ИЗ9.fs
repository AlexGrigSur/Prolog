...
// Вариант №2
// Первое найденное число: 134043 (см.ИЗ9 Результат.PNG)
...
let rec razloj number =
    let rec razlojPobolshe number delitel previous count =
                ...
                if(prime number) then 
                    ...
                    if(number=1) then count
                            ...
                            if(prime delitel = true && number%delitel=0) then 
                                if (delitel=previous) then razlojPobolshe (number/delitel) delitel previous (count)
                                    else
                                        razlojPobolshe (number/delitel) delitel delitel (count+1)
                            else
                                razlojPobolshe number (delitel+1) delitel count
    razlojPobolshe number 2 1 0

let rec perebor number limit count =
    if(count=limit) then number;
        else
            let temp = razloj number 
